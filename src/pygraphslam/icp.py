...
def compute_C_k(point1, point2):
    """Compute the covariance matrix for a pair of points."""
    d = point1 - point2
...
def vers(theta):
    """Compute the versor for a given angle."""
    return np.array([np.cos(theta), np.sin(theta)])


def compute_covariance(laser_ref, laser_sens, t, theta, angles):
    """Compute the covariance matrix for a given laser senor measurement and pose."""
    ...
    d2J_dx2 = np.vstack(
        [np.hstack([d2J_dt2, d2J_dt_dtheta]), np.hstack([d2J_dt_dtheta.T, d2J_dtheta2])]
    )
...
def best_fit_transform(A, B):
    ...
    Finds the transform that maps corresponding points A to B in m spatial dimensions.
    Input:
...
def icp(A, B, init_pose=None, max_iterations=20, tolerance=0.001):
    ...
    for i in range(max_iterations):
        ...
        T, _, _ = best_fit_transform(
            filtered_src[:m, distances < 1.0].T, dst[:m, indices].T
        )
